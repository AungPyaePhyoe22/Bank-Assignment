...
void finding_phone_number(unsigned int to_find);
...
// from-WinHtut-to-lonelone-100
    ...
    if(who == 't'){
        ...
        for(int i=0; i<5; i++){
            db[transfer].trc[space_array[transfer]-14].note[i] = from[i];
            index_point++;
        }
        //int aaa=0;
        ...
        for(int a=0; a<trans_name_counter; a++){
            ...
            db[transfer].trc[space_array[transfer]-14].note[index_point]=db[transfer].name[a];
            index_point++;

        }

        for(int a=0; a<4; a++){
            db[transfer].trc[space_array[transfer]-14].note[index_point]=to[a];
            index_point++;
        }
        for (int aaa = 0; aaa < receive_name_counter; ++aaa) {
            db[transfer].trc[space_array[transfer]-14].note[index_point]=db[receiver].name[aaa];
            index_point++;

        }

        db[transfer].trc[space_array[transfer]-14].note[index_point]='$';
        index_point++;
        for(int aaa=0; aaa<amount_counter; aaa++){
            db[transfer].trc[space_array[transfer]-14].note[index_point]=int_to_char_array_data[aaa];
            index_point++;
        }
        ...
        for(int i=0; i<25; i++){
            ...
            db[transfer].trc[space_array[transfer]-14].note[index_point]=Ctime[0].c_time[i];
            index_point++;
        }
        ...
        for(int a=0; a<receive_name_counter; a++){
            db[receiver].trc[space_array[receiver]-14].note[index_point]=db[receiver].name[a];
            index_point++;

        }
        for(int a=0; a<14; a++){
            db[receiver].trc[space_array[receiver]-14].note[index_point]=rec[a];
            index_point++;
        }
        for(int a=0; a<trans_name_counter; a++){
            db[receiver].trc[space_array[receiver]-14].note[index_point]=db[transfer].name[a];
            index_point++;
        }
        db[receiver].trc[space_array[receiver]-14].note[index_point]='$';
        index_point++;
        for(int aaa=0; aaa<amount_counter; aaa++){
            db[receiver].trc[space_array[receiver]-14].note[index_point]=int_to_char_array_data[aaa];
            index_point++;
        }
        ...
        get_time();
        for(int a=0; a<25; a++){
            db[receiver].trc[space_array[receiver]-14].note[index_point]=Ctime[0].c_time[a];
            index_point++;
        }
...
void loadingAllDataFromFile(){
    ...
    if( fptr == NULL){
        ...
        for(int ncc=0; ncc<SIZE ; ncc++){
            ...
            for(int gcc=0; gcc<space_array[ncc]-14 ; gcc++){
...
void recording_alldata_toFile() {
    ...
    for (int ncc = 0; ncc < users; ncc++) {
        fprintf(fptr, "%u%c%s%c%s%c%s%c%s%c%s%c%s%c%u%c%u%c%f%c%s%c%d%c%llu%c%u%c%s%c%d", db[ncc].id, ' ', db[ncc].name,
                ' ', db[ncc].nrc, ' ', db[ncc].email, ' ', db[ncc].password,
                ' ', db[ncc].pOrb, ' ', db[ncc].loan_s, ' ', db[ncc].monthly_income, ' ', db[ncc].loan_amount, ' ',
                db[ncc].loan_rate, ' ', db[ncc].acc_s, ' ', db[ncc].acc_level,
                ' ', db[ncc].phNumber, ' ', db[ncc].cur_amount, ' ', db[ncc].address, ' ', db[ncc].transAmoLimitPerDay);
